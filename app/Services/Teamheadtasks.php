...
class Teamheadtasks {
        ...
        foreach ($alltasks as &$task) {
            ...
            if (!isset($groupData[$taskId])) {
                ...
                $groupData[$taskId] = [
                    ...
                    'completed_activities' => $this->taskStaffActivityModel->where(['task_id' => $task['id'],'status' => 'Completed'])->groupBy('task_activity_id')->countAllResults(),
                    'priority'  => $task['priority'],
                    'status'    => $task['status'],
                    'overdue_date' => $task['overdue_date'],
                    'progress'  => $task['progress'],
    ...
    function getActivities($taskId=false,$search=false,$filter=false,$startDate=false,$endDate=false,$staffId=false){
        if(hasRole() == 3){
             return  $this->activityModel->getActivities($taskId,$search,$filter,$startDate,$endDate,'');
        }else{
