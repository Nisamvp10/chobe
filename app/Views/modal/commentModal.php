    ...
    <div class="bg-white rounded-lg p-6 w-full max-w-md">
        ...
        <h3 class="text-lg font-semibold text-gray-900">Update Comment</h3>
        <button data-close="commentModal" class="text-gray-400 hover:text-gray-500">
        ...
        <!-- Form -->
        <form id="commentForm" method="post" action="<?= base_url('comment/update') ?>">
            <?= csrf_field() ;?>
            <div class="mb-4">
                <label for="comment" class="block text-sm font-medium text-gray-700 mb-1">Comment</label>
                <input type="hidden" id="taskId" name="taskId">
                <input type="hidden" id="activityId" name="activityId">
                <textarea id="comment" name="comment" class="w-full px-3 py-2 border border-gray-300 rounded-lg focus:ring-2 focus:ring-blue-500 focus:border-blue-500" placeholder="Enter comment" required></textarea>
                 <div class="invalid-feedback" id="comment_error"></div>
            </div>

            <div class="flex justify-end gap-3">
                <button type="button" data-close="commentModal" class="px-4 py-2 text-gray-700 border border-gray-300 rounded-2 hover:bg-gray-50">Cancel</button>
