    ...
    <div class="flex items-center justify-between">
        <div class="col-lg-12">
            <div class="d-flex justify-content-between align-items-center mb-0">
                ...
                <?php if(haspermission(session('user_data')['role'],'create_category')) : ?>
                <div>
    ...
    <div class="bg-white rounded-lg p-6 w-full max-w-md">
        ...
        <form id="categoryForm" method="post" action="<?= base_url('category/save') ?>">
            ...
            <div class="flex justify-end gap-3">
                <button type="button" onclick="closeModal()" class="px-4 py-2 text-gray-700 border border-gray-300 rounded-2 hover:bg-gray-50">Cancel</button>
                    ...
                    categories.forEach(category => {
                        html += `
                            <tr class="hover:bg-gray-50 ${category.is_active ==0 ? 'bg-red-100 bg-opacity-50' :'' }"  >
                                ...
                                <td class="px-2 py-2 whitespace-nowrap">
                                    <div class="text-sm text-gray-900">${category.category}</div>
                                </td>
                              
                                <td class="px-2 py-2 whitespace-nowrap text-right text-sm font-medium">
                                ${category.is_active ==1 ?  `<a href="<?=base_url('category/edit/');?>${category.encrypted_id}" class="text-blue-600 hover:text-blue-800 mr-3">View</a>` :`<span data-id="${category.encrypted_id}" onclick="unlockCategory(this)" class="text-blue-600 hover:text-blue-800 mr-3 cursor-pointer "><i class="bi bi-lock"></i></span>` }
