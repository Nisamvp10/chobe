            ...
            <div class="space-y-4">
                ...
                <!-- open branches -->
                 <div>
                    ...
                    <div id="branchTags" class="flex flex-wrap gap-2 mb-2">
                    <?php foreach ($selectedSpecialties as $sp): ?>
                        <div class="inline-flex items-center bg-blue-50 text-blue-700 rounded-full px-3 py-1 text-sm">
                            <?= esc($sp['category']) ?>
                            <button type="button" class="ml-1 text-blue-700 hover:text-blue-900 removeBtn" data-value="<?= $sp['id'] ?>" data-text="<?= $sp['category'] ?>">
                    ...
                    <!-- Dropdown + Add button -->
                    <div class="flex">
                        <select id="projectDropdown" class="flex-1 py-2 px-3 border border-gray-300 rounded-l-lg focus:ring-2 focus:ring-blue-500 focus:border-blue-500">
                        <option value="">Select a service</option>
                        <?php
                        ...
                        if ($allProjects) {
                            ...
                            foreach ($allProjects as $items) {
                                if (!in_array($items['id'], $selectedIds)) {
                            ?>
                                <option value="<?= $items['id'] ?>"><?= $items['project'] ?></option>
                            <?php
                                }
                            }
                        } ?>
                        ...
                        <button type="button" id="addProjectBtn" class="px-3 py-2 bg-blue-600 text-white rounded-r-md hover:bg-blue-700 transition-colors">
                        <svg xmlns="http://www.w3.org/2000/svg" width="18" height="18" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round" class="lucide lucide-plus "><path d="M5 12h14"></path><path d="M12 5v14"></path></svg>
                        </button>
                    </div>
                </div>
...
// REMOVE SERVICE
serviceTags.addEventListener('click', (e) => {

    const btn =
        e.target.closest('.removeServiceBtn');
...
const branchTags =
    document.getElementById('branchTags');


// ADD PROJECT
addProjectBtn.addEventListener('click', () => {

    const value = projectDropdown.value;

    const text =
        projectDropdown.options[
            projectDropdown.selectedIndex
        ].text;
    ...
    tag.className =
        'inline-flex items-center bg-green-50 text-green-700 rounded-full px-3 py-1 text-sm';

    tag.innerHTML = `
        ${text}

        <button type="button"
            class="ml-1 text-green-700 hover:text-green-900 removeProjectBtn"
            data-value="${value}"
            data-text="${text}">
...
// REMOVE PROJECT
branchTags.addEventListener('click', (e) => {

    const btn =
        e.target.closest('.removeProjectBtn');
