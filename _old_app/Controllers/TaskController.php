...
class TaskController extends Controller {
 ...
 private function newtaskAssign($taskType) {
    ...
    function myTaskList() {
        ...
        //
        $projectUnitIds = userProjects();
        if(!empty($projectUnitIds)){
            $alltasks = $this->serviceCommon->userProjectsTasks('','',$notifiytask,$filter,$search);
        }else{
        ...
        if(!empty($alltasks)){
            foreach ($alltasks as &$task) {
                ...
                if (!isset($groupData[$taskId])) {
                    ...
                    $groupData[$taskId] = [
                        ...
                        'branch_name' => $task['branch_name'],
                        'project'   => $task['project_id'],
                        'total_activities' => $this->serviceCommon->countTaskActivities($task['id'],session('user_data')['id']),
            ...
            return $this->response->setJSON([ 'success'=>false,'task' => [],'completedTasks'=>0,'pendingTasks' =>0]);
        }
