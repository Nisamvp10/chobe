...
function loadTask(search = '') {
    ...
    $.ajax({
        ...
        url: App.getSiteurl() + 'task/team-head-task',
        type: "GET",
    ...
    if (tasks.length === 0) {
        ...
        <div class="bg-white draggable-task___ rounded-lg shadow-sm p-4 cursor-pointer hover:shadow-md transition-shadow duration-200 border-l-4 border-orange-500 draggable-task___" 
            ...
            <div class="flex justify-between items-start mb-2">
                <h6 class="font-medium text-gray-800 truncate flex-1 text-capitalize">${task.branch_name} [${task.oracleCode}]</h6>
            ...
            ${task.usertype === 'single'
                            ? `
                ${task.users?.img
                                ? `
                        <div class="relative rounded-full overflow-hidden flex items-center justify-center w-10 h-10 text-xs border-2 border-white">
                            <img src="${task.users.img}" alt="${task.users.staffName}" class="w-full h-full object-cover">
                        </div>
                        <span>${task.users.staffName}</span>
                    `
                                : `
                        <div class="relative flex items-center gap-2">
                            <div class="relative rounded-full overflow-hidden flex items-center justify-center w-10 h-10 text-xs border-2 bg-blue-100 border-white">
                                <span class="text-blue-600 font-medium">
                                    ${task.users?.staffName?.charAt(0) || '?'}
                                </span>
                            </div>
                            <span class="4">${task.users.staffName}</span>
                        </div>
                    `
                            }
            `
